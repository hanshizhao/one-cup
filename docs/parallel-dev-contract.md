...
> **新会话开工前必读。** 本文件约定两个并行 worktree 同时开发时的资源分配、
> 共享文件修改规则、以及合并回 main 的标准操作。
>
> 最后更新：2026-07-04 ｜ 适用 worktree：`feat/unit-mgmt`、`feat/color-mgmt`
...
- 有一批"集中式共享文件"两边都要改——**严格遵守本文件第 3 节的修改规则**，
  尤其是种子 Guid 分配（第 3.1 节），**不许抢占对方已分配的 Guid 段**。
- 合并策略走 **方案 B**：开发期两边互不干扰各自做完，合并时统一处理 EF 冲突（第 4 节）。
...
两个 worktree 均基于 `main@87bf3a9` 创建。`main` 是只读基线，只用于最终合并。

**新会话开工第一步**：在新 worktree 目录里确认分支正确
```bash
git branch --show-current   # 应为 feat/unit-mgmt 或 feat/color-mgmt
```
...
`SeedData.cs` 用确定性 Guid，**第 4 段是按区间分配的**。两边已划定互不重叠的区间：

| 资源类别 | 单位管理（unit） | 颜色管理（color） | 已占用（不可碰） |
| --- | --- | --- | --- |
| 权限 Guid（`...000000012X`） | **121–123** | **不复用**（color 权限已有 `109/110`） | 101–117 已用 |
| 目标类型 Guid（`...000000022X`） | **211** | **不复用**（color 目标类型已有 `205`） | 201–206 已用 |
...
> 避免开发中途有人往 118–120 塞东西导致撞车。**118–120 现阶段两边都不许用。**
...
两边各自加迁移时，**时间戳必须不同**。EF 的 `migrations add` 会自动生成时间戳，
但为确保不撞，约定：
...
EF 时间戳精确到秒，两边不太可能同一秒执行，但**若真的撞了**，手动改一个文件名的时间戳
后缀（改大），确保两份迁移在合并后能按时间线排列。
...
**核心思路**：开发期两个 worktree 互不通信、互不 rebase，各自独立做完。合并时，
**先合一个进 main，另一个再 merge main 解决冲突**。

### 4.1 谁先合并？

**建议单位管理先合**（颜色管理复用已有种子，改动面略小、冲突处理更简单，放后面更稳）。
但这不是强制的——谁先开发完谁先合。

### 4.2 第一个分支合并（以 unit 为例）

```bash
cd "C:/Users/mi/Desktop/work_space/one-cup"   # 回主仓库
git checkout main
git merge --no-ff feat/unit-mgmt
# unit 分支没有冲突源，直接合入
git push origin main   # 如果有远端
```

### 4.3 第二个分支合并（color 后合）

```bash
cd "C:/Users/mi/Desktop/work_space/one-cup/.worktrees/color-mgmt"
git fetch ../..  main:main-local   # 拿到合了 unit 之后的 main
...
| 文件 | 解决方式 |
| --- | --- |
| `SeedData.cs` | 两边 Guid 段不重叠（按 3.1 分配），保留两边的常量块即可 |
| `OneCupDbContext.cs` | DbSet / ApplyConfiguration / Seed() 各自有独立区块，保留两边 |
| `OneCupDbContextModelSnapshot.cs` | 🔴 **必冲突**。见 4.4 专门处理 |
| `{timestamp}_AddUnitModule.cs` vs `{timestamp}_AddColorModule.cs` | 时间戳不同则不冲突；撞名则改一个时间戳 |
| `routes.ts` / `router.tsx` / `locale/index.ts` | 数组/对象追加，保留两边 |
...
种子 Guid 第 4 段分配：
  ...
  212+     颜色管理额外目标类型备用（205=TargetTypeColor 已存在）

EF 迁移命名：
  单位：AddUnitModule    颜色：AddColorModule
```
