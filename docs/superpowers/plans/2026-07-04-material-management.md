...
**Interfaces:**
- Produces: `Material` 实体类(`: BaseEntity`),字段 `Code/Name/Spec/Category/UnitId/Remark/SortOrder/IsActive`。供 Task 3(Spec)、Task 4(Service)、Task 5(Configuration)、Task 7(测试)、Task 8(DbContext)使用。
...
Expected: BUILD SUCCEEDED(此时实体/配置未被 DbContext 发现,但能编译通过;DbSet 在 Task 6 加)。
...
**Interfaces:**
- Produces: `CreateMaterialRequest` / `UpdateMaterialRequest` / `UpdateMaterialStatusRequest` / `MaterialDto`。供 Task 4(Service)、Task 5(Controller)使用。
- Consumes: 无。
...
**Interfaces:**
- Produces: `MaterialFilterSpec` / `MaterialPagedSpec` / `MaterialActiveSpec` / `MaterialByIdSpec` / `MaterialByCodeSpec`。供 Task 4(Service)、Task 8(测试)使用。
...
## Task 4: IMaterialService 接口

**Files:**
- Create: `backend/src/OneCup.Application/Interfaces/IMaterialService.cs`

**Interfaces:**
- Produces: `IMaterialService`(7 方法签名)。供 Task 5(实现)、Task 6(Controller)、Task 9(DI 注册)使用。
...
## Task 5: MaterialService 实现

**Files:**
- Create: `backend/src/OneCup.Application/Services/MaterialService.cs`

**Interfaces:**
- Produces: `MaterialService` 类(实现 `IMaterialService`)。供 Task 6(Controller)、Task 9(DI)、Task 8(测试)使用。
- Consumes:
  ...
  - `Material` 实体(Task 1)、`MaterialDtos`(Task 2)、`MaterialSpecs`(Task 3)
...
public class MaterialService : IMaterialService
{
    ...
    public MaterialService(IRepository<Material> materials, IUnitOfWork uow, INumberingService numbering)
    {
    ...
    public async Task UpdateMaterialAsync(Guid id, UpdateMaterialRequest request, CancellationToken ct = default)
    {
        ...
        if (request.Spec is not null) entity.Spec = request.Spec;
...
## Task 6: MaterialController

**Files:**
- Create: `backend/src/OneCup.Api/Controllers/MaterialController.cs`

**Interfaces:**
- Produces: `MaterialController`(HTTP 端点,路由 `api/materials`)。供 Task 9 前端 API 调用。
- Consumes: `IMaterialService`(Task 4/5)、各 DTO(Task 2)。权限策略 `material:read/create/update/delete`(已在 main)。
...
## Task 7: DbContext 加 DbSet + Program.cs 加 DI
...
## Task 8: EF 迁移 AddMaterialModule
...
## Task 9: MaterialService 单元测试

**Files:**
- Create: `backend/tests/OneCup.UnitTests/Material/MaterialServiceTests.cs`

**Interfaces:**
- Consumes: `MaterialService`(Task 5)、`Material` 实体、各 DTO(Task 2)、`INumberingService`/`IUnitOfWork`/`IRepository<T>`、`Repository<T>`/`UnitOfWork`(真实实现)、`OneCupDbContext`、`NumberTargetTypes.Material`、`DomainException`。
...
public class MaterialServiceTests
{
    private static (OneCupDbContext db, MaterialService svc, FakeNumberingService numbering) Setup()
    {
        ...
        var svc = new MaterialService(new Repository<Domain.Entities.Material>(db), new UnitOfWork(db), numbering);
...
## Task 10: MaterialSpecs 单元测试

**Files:**
- Create: `backend/tests/OneCup.UnitTests/Material/MaterialSpecsTests.cs`

**Interfaces:**
- Consumes: `MaterialSpecs`(Task 3)、`Material` 实体、`Repository<T>`、`OneCupDbContext`。
...
Run: `dotnet test backend/OneCup.sln --filter "FullyQualifiedName~OneCup.UnitTests.Material"`
Expected: 全部 PASS(MaterialService 13 + MaterialSpecs 7 = 20 个测试用例)
...
**Interfaces:**
- Produces: `MaterialListItem` / `MaterialDetail` / `MaterialFormData` / `MaterialQuery` / `MaterialPagedResult` 类型 + `getMaterials`/`getMaterial`/`createMaterial`/`updateMaterial`/`deleteMaterial`/`updateMaterialStatus` 方法。供 Task 12-14(前端页面)使用。
...
- Consumes: `@/api/material`(Task 11)、`@/api/measurementUnit.getAllActiveUnits`(已有,返回 `MeasurementUnit[]`,字段 `id`/`nameZh`)、`@/components/PermissionWrapper`、`@/utils/useLocale`、`./form`、`./detail`、`./locale`、`./style`。
...
- Consumes: `@/api/material`(Task 11)、`@/api/numbering.previewCode`(已有)、`@/api/measurementUnit.MeasurementUnit`(类型)、`@/utils/useLocale`、`./locale`。严格遵守 c02。
...
```
Expected: BUILD SUCCEEDED + 全部测试 PASS(新增 20 + 既有无回归)
...
- §2.1 DTO → Task 2 ✅
- §2.2 Specs(5 个) → Task 3 ✅
- §2.3 IMaterialService/MaterialService(7 方法 + 事务取号 + 物理删除) → Task 4/5 ✅
- §2.4 Controller(7 端点 + 单数文件名 + 复数路由) → Task 6 ✅
- §2.5 EF 配置(已含 Task 1) → Task 1 ✅
- §2.6 迁移 AddMaterialModule → Task 8 ✅
- §4.1 共享文件(DbContext/Program) → Task 7 ✅
- §3 前端(API/列表/表单 c02/详情/样式/locale) → Task 11-15 ✅
- §4.4 单测(Service + Specs) → Task 9/10 ✅
- §4.3 验证 → Task 16 ✅
- 无遗漏。

**2. Placeholder scan:** 全部代码块完整,无 TBD/TODO/"类似 Task N"。每步含确切文件路径、完整代码、确切命令与预期输出。✅

**3. Type consistency:**
- `Material` 实体字段(Code/Name/Spec/Category/UnitId/Remark/SortOrder/IsActive)在 Task 1/2/3/5/9/10 一致。
- `MaterialService` 构造函数 `(IRepository<Material>, IUnitOfWork, INumberingService)` 在 Task 5(定义)和 Task 9(测试 new)一致。
- `Remove(T)` 方法名(Task 5 用 `_materials.Remove(entity)`)与 `IRepository<T>` 签名一致(非 DeleteAsync)。
- `GenerateAsync` 返回 `string`(非 nullable),Task 5 接收为 `var code` 一致。
- 前端 `MaterialDetail`/`MaterialFormData` 在 Task 11(定义)/12(index)/13(form)/14(detail)一致。
- `unitMap` prop 从 Task 12(index)传给 Task 14(detail),类型 `Record<string,string>` 一致。
- `units: MeasurementUnit[]` prop 从 Task 12 传给 Task 13,字段用 `u.nameZh`/`u.symbol`/`u.id` 与 `measurementUnit.ts` 一致(已确认无 `name` 字段)。✅
